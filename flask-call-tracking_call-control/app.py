...
from Model.database_queries import db_fetch_data, \
    ...
    db_call_insert
from Model.telnyx_commands import telnyx_number_acquire, \
...
@app.route("/call/delete/<id>/", methods=['GET', 'POST'])
def delete_call(id):
    db_call_delete(id)
...
def handle_call_answered(call):
    ...
    purchased_number = body["data"]["payload"]["to"]
    number_to_forward_to = db_number_forward_fetch(purchased_number)
...
def inbound_call():
    ...
    try:
        if event_type == "call.initiated":
            ...
            handle_call_answered(call)
        elif event_type == "call.hangup":
            print(body)
            cnam_info = telnyx_cnam_lookup(calling_number)
            # time difference
            end_time = ''.join(body.get(
                "data").get(
                "payload").get(
                "end_time"))
            start_time = ''.join(body.get(
                "data").get(
                "payload").get(
                "start_time"))
